    ...
    @testset "BibParser.jl" begin
        ...
        @testset "BibTeX" begin
            ...
            @testset "don't copy fields from previous entries (#28)" begin
                ...
                @test parsed["FroeseFischer1997"].authors !=
                      parsed["Javanainen1988"].authors
        ...
        @testset "BibLaTeX validation" begin
            ...
            @test_throws "missing required field {date|year}" BibParser.parse_string(
                missing; format = :BibLaTeX)
            parsed = @test_logs (:warn, r"missing required field \{date\|year\}") BibParser.parse_string(
        ...
        @testset "format detection and diagnostics" begin
            ...
            @test parse_bibliography("""[{"id":"x","type":"book","title":"T"}]""").format ==
                  :CSL
            ...
            @test !isempty(parse_bibliography(
                "<xml><record>"; format = :EndNote).diagnostics)
